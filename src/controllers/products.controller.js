...
import { generateProduct } from '../helpers/utils.js';
...
export const findById = async (req, res) => {
  try {
    ...
    if (!result) {
      res
        .status(200)
        .send({ status: 'success', message: 'El producto No existe.' });
    } else {
    ...
    res
      .status(500)
      .send({ error: error, message: 'No se pudo obtener el product.' });
  }
};
...
export const findByTitle = async (req, res) => {
  try {
    ...
    const result = await productService.findByTitle(title);
    if (!result) {
      res
        .status(200)
        .send({ status: 'success', message: 'El producto No existe.' });
...
export const save = async (req, res) => {
  try {
    ...
    if (result.code === 11000) {
      ...
      return res.status(409).send({ error: 'Valor duplicado en campo único' });
    }
    res.status(200).send({ status: 'success', payload: result });
  } catch (error) {
    res
...
export const getProducts = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .send({ error: error, message: 'No se pudo Generar productos.' });
  }
};
